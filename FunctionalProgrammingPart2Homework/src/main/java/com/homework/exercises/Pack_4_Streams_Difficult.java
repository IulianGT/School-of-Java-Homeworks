...
public class Pack_4_Streams_Difficult {
    ...
    public void exercise_1_findFirst() {
        ...
        result = EMPLOYEES.stream()
                        .map(empl -> empl.getFirstName() + " " + empl.getSurname())
                                .filter(empl -> {
                                    boolean b = (int) EMPLOYEES.stream().map(e -> e.getFirstName() + " " + e.getSurname())
                                            .filter(e -> empl.equals(e)).count() > 1;
                                    return b;
                                }).findFirst().orElse("There is no match");
    ...
    public void exercise_2_groupingBy_counting() {
        ...
        result = EMPLOYEES.stream()
                .collect(Collectors.groupingBy(empl -> empl.getHomeAddress().getPostCode().substring(0,2), Collectors.toList()))
                .values().stream()
                .filter(employees -> employees.size() >=5).count();
    ...
    public void exercise_3_flatMap() {
        ...
        result = EMPLOYEES.stream()
                        .flatMap(empl -> Stream.of(empl.getHomeAddress(), empl.getCorrespondenceAddress().get()))
